...
def find_match(source, template, next_img, prev_img, next_gray, prev_gray, ori_img, ori_prev, ds_factor, method, half_template_size, yy_ul, yy_lr, xx_ul, xx_lr, x,y, savefilename): 
    ...
    if len(loc)>0:
        ...
        for pt in match_loc: 
            ...
            prev_block = source[match_ul_x:match_ul_x+w+1,match_ul_y:match_ul_y+w+1]
            next_block = template

            prev_gray_cp =  prev_gray.copy()
            next_gray_cp =  next_gray.copy()

            cv.rectangle(prev_gray_cp, (y-half_template_size+match_ul_y, x-half_template_size+match_ul_x),\
                                    (y+half_template_size+match_ul_y,x+half_template_size+match_ul_x), 255)
...
def plot_match(prev_file, next_file, ori_prev, ori_file, ds_factor, savefilename, half_template_size=2, half_source_size=4, method = 'hog'): 
    ...
    x, y = int(24*8./ds_factor), int(26*8./ds_factor)
    ...
    (x, y, prev_match_x, prev_match_y) = find_match(source, template, next_img, prev_img, next_gray, prev_gray, \
                                        ori_img, ori_prev, ds_factor, method, half_source_size,\
...
if method == 'hog': 
    savefilename = savefilepath+'HOG/'+'step3_final_step.png'
else:
    savefilename = savefilepath+'TM_CCORR/'+'step3_final_step.png'
